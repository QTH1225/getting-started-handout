...
\begin{itemize}
  ...
  \item 浏览器类
    \begin{itemize}
      ...
      \item Zen Browser：基于 Firefox 的轻量浏览器，界面更加简洁美观，同时支持自定义主题和插件。
    \end{itemize}
  \item 压缩与解压缩类
    \begin{itemize}
  ...
  \item 输入类
    \begin{itemize}
      \item Rime：一个高度可定制的输入法框架，可以自定义输入方式与习惯。
      \item InputTip/ImTip：输入状态提示，可以显示当前处于中、英文状态等，前者也可以根据不同输入状态更换鼠标样式。
      \item Keyviz/KeyCastOW：键鼠操作可视化，会在屏幕中显示按键操作，方便用户查看和学习。
      \item ContextMenuManager：通过可视化方式管理基于注册表的右键菜单，避免菜单栏过长。

    \end{itemize}
  \item 文件搜索与窗口管理
    \begin{itemize}
      \item Everything/Everything Toolbar：一个免费的文件搜索工具，能够快速地搜索计算机上的文件。它的搜索速度极快，支持多种搜索方式，包括模糊搜索、正则表达式搜索等，后者是任务栏集成的版本，操作更加方便。
      \item Fences/Coodesker：桌面整理工具，可以按照文件类型进行整理。
      \item Deskpins：任意窗口置顶，操作十分简单。
      \item AltSnap：按住Alt可在任意位置拖动窗口。
      \item Groupy：任意窗口的分组管理，可以将不同的窗口在上方显示与浏览器类似的标签条，方便窗口管理。
      \item Office Tab：Office三件套的窗口管理，在工具栏下方显示所有打开的文档，方便切换与关闭。
      \item OneCommander/Directory Opus：更高级的文件管理，具有批量命名、批量复制、批量移动等功能，操作更加方便。
    \end{itemize}
  \item 效率与自动化
    \begin{itemize}
      \item uTools：极简的多功能应用，具有截图翻译、提取颜色、转换图片格式等插件，也有插件市场可以选择。
      \item AutoHotkey：一款功能强大的自动化脚本工具，可以很方便地完成热键修改、文本替换、自定义宏等操作。
      \item PowerToys：Windows 官方效率工具合集，包括保持唤醒、颜色选择器、键盘管理器以及鼠标工具等工具。
    \end{itemize}
  \item 其他
    \begin{itemize}
      ...
      \item TrafficMonitor：网速/CPU/内存悬浮窗，也可以嵌入任务栏中实时显示内存与CPU运行情况。
    \end{itemize}
\end{itemize}
